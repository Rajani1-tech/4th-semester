...
void scan(int *requests, int n, int head, char direction){ int total_movement = 0; 
int current_track; 
int i, j; 
...
head = 0; 
for (i = head_index + 1; i < n; i++) { current_track = requests[i]; 
printf("%d -> ", current_track); 
total_movement += abs(head - current_track); head = current_track; 
} 
} 
...
head = n - 1; 
for (i = head_index - 1; i >= 0; i--) { current_track = requests[i]; 
printf("%d -> ", current_track); 
total_movement += abs(head - current_track); head = current_track; 
} 
} 
